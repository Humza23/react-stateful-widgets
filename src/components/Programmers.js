...
export default function Programmers() {
  ...
  const [idFeatured, setIdFeatured] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return programmerList[idFeatured - 1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idFeatured === null ? 'royalblue' : 'gold' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmerList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setIdFeatured(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          idFeatured ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
